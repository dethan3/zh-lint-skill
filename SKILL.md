...
1. 运行 `<skill-dir>/zh-lint.sh --export-review review.jsonl [path...]`
   提取中文文本段并生成审查文件。
...
5. 运行 `<skill-dir>/zh-lint.sh --apply-review review.jsonl` 应用已批准的修改。
...
- `zh-lint.sh`：**唯一入口脚本**（Agent 应始终通过此脚本调用，不要直接调用 scripts/ 下的文件）
